...
export default class SceneCharacterSelection extends Phaser.Scene {
  ...
  cam.fadeIn(1000);
  		...
  		if (scene.game.online && (disconnectCounter < 3)) {
  			$.ajax({
  		        ...
  		        if (numPlayers < 3) {
  		        	scene.time.addEvent({
  		        		delay: 500,
  		      			callback: () => {LoadScene(scene, 'menu'); document.getElementById("chatArea").innerHTML += "Someone left the server. <br />"}
  		        	});
  		        } else {
  		        	for (var i = 0; i < 3; i++) {
  		        		DeactivateRole(i);
    ...
    getServerInfo(this, this.game.serverIP)
  }
