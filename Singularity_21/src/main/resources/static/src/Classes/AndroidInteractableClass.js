...
class AndroidInteractableClass {
  constructor(scene, existingObj, mainObject, xObb, yObj, sprObj, sclObj, includeActivator = false, xAct = 0, yAct = 0, sprAct = "generic", sclAct = 0, follow = false, followXoffset = 0, followYoffset = 0) {
    ...
    game.android1.cursors.coop.on('down', function (event) {
    ...
    game.android2.cursors.coop.on('down', function (event) {
...
class DoorTimer extends AndroidInteractableClass {
  ...
  objectActivate() {
    ...
    if (this.objectiveY = this.startPosY) {
      ...
      function reset(obj) { obj.objectiveY = obj.startPosY; obj.activator.setFrame(0)}
    }
  }
  update(time, delta) {
    super.update();
...
class InteractiveBlade4 extends AndroidInteractableClass {
  constructor(scene, blade, xObb, yObj, xAct, yAct, distance) {
    super(scene, true, blade, 0, 0, "", 1, true, xAct, yAct, "pressurePlate", 1);
...
class AndroidInteractablesArray {
  ...
  initializeScene3(eSurfaces, doors) {
    ...
    this.items[14] = new ElectricSurfInteractable(this.scene, eSurfaces[0], 1586, 496);
  ...
  //Inicializamos la escena 4 (nivel 3).
    initializeScene4(doors, blade) {
      ...
      this.items[2] = new InteractiveBlade4(this.scene, blade, 2430, 102, 2580, 114, 260, doors[2]);
      this.items[3] = new DoorTimer(this.scene, doors[2], 2514, 209, -100);
